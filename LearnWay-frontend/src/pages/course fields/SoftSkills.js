...
const SoftSkills = () => {
    ...
    useEffect(() => {
        ...
        axios.get('/api/course/')
            .then(response => {
                // Filter courses to include only those with category ID 3
                console.log(response.data);
                const softSkillsCourses = response.data.filter(course => course.category.catId === 3);
                setCourses(softSkillsCourses);
                console.log(softSkillsCourses);
            })
            .catch(error => {
    ...
    return (
        <Container>
            <h1 className="my-4 text-center">Soft Skills Courses</h1>
            <Row className="mb-4">
                <Col md={8}>
                    <Form.Control
                        type="text"
