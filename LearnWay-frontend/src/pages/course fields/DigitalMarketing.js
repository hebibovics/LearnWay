...
const DigitalMarketing = () => {
    ...
    useEffect(() => {
        ...
        axios.get('/api/course/')
            .then(response => {
                ...
                const dmCourses = response.data.filter(course => course.category.catId === 8);
                setCourses(dmCourses);
                console.log(dmCourses);
            })
            .catch(error => {
    ...
    return (
        <Container>
            <h1 className="my-4 text-center">Digital Marketing Courses</h1>
            <Row className="mb-4">
                <Col md={8}>
                    <Form.Control
                        type="text"
